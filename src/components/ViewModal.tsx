...
export function ViewModal({ isOpen, onClose, employee, onAjustarPonto }: ViewModalProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <div className="sit-panel relative w-full max-w-3xl overflow-hidden shadow-2xl border-brand-accent/20 transition-all duration-300">
        ...
        {/* Header Section */}
        <div className="flex items-center justify-between border-b border-brand-border/40 px-6 py-5 bg-gradient-to-b from-black/20 to-transparent shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-brand-accent/15 border border-brand-accent/30 text-brand-accent">
          ...
          <button
            onClick={onClose}
            className="rounded-xl p-2.5 bg-white/5 border border-white/5 text-brand-muted hover:bg-white/10 hover:border-white/15 hover:text-white transition-all duration-200 cursor-pointer"
            aria-label="Separar Modal"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="px-6 py-6 bg-black/15 border-b border-brand-border/30">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="relative group shrink-0">
              ...
              <div className="relative h-28 w-28 sm:h-32 sm:w-32 overflow-hidden rounded-full border-4 border-brand-panel-light bg-black/40 flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-[1.02]">
                {employee.foto ? (
                  <img src={employee.foto} alt={employee.nome} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                ) : (
                  <div className="flex flex-col items-center gap-1.5">
                    <UserRound className="h-12 w-12 text-brand-accent" />
                  </div>
                )}
              </div>
              {employee.autorizadoDirigir && (
                <div 
                  className="absolute -bottom-1 -right-1 h-10 w-10 bg-emerald-500 border-4 border-brand-panel rounded-full flex items-center justify-center text-white shadow-xl shadow-emerald-900/35 transition-transform duration-300 hover:scale-110" 
            ...
            <div className="text-center sm:text-left flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <h4 className="typ-hero text-xl sm:text-2xl font-black text-white leading-tight drop-shadow-sm">{employee.nome}</h4>
                {employee.id && (
                  <span className="self-center sm:self-auto bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 px-2 py-0.5 rounded-full text-[9px] uppercase tracking-wider font-extrabold flex items-center gap-1 shrink-0">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
                    Ativo
                  </span>
                )}
              </div>
              <p className="text-brand-accent font-extrabold text-[12px] uppercase tracking-wider mt-1.5 flex items-center justify-center sm:justify-start gap-2">
                <Sparkles className="w-4 h-4 text-brand-accent animate-pulse" />
                {employee.especialidade || 'Função Não Definida'}
              </p>

              {/* Verified badges */}
              <div className="mt-4 flex flex-wrap gap-2.5 justify-center sm:justify-start">
                <div className="inline-flex items-center gap-1.5 bg-black/25 px-3 py-1.5 rounded-lg border border-white/5 text-[11px] font-mono font-semibold text-brand-muted hover:border-brand-accent/20 transition-colors">
                  <span className="text-[10px] text-brand-accent">MATRÍCULA:</span>
                  <span className="text-white font-bold">{employee.matricula || '-'}</span>
                  <button 
                    onClick={() => handleCopy(employee.matricula || '', 'Matrícula')}
                    className="p-1 hover:bg-white/5 rounded-md text-brand-muted hover:text-white transition-colors ml-1"
                ...
                {employee.empresa && (
                  <span className="bg-brand-accent/10 border border-brand-accent/20 text-brand-accent px-3 py-1.5 rounded-lg text-[10px] uppercase font-black tracking-widest flex items-center gap-1.5">
        ...
        {/* Dynamic segment tabs containing interactive state */}
        <div className="flex border-b border-brand-border/40 bg-black/10 px-6 py-2 gap-1.5 sm:gap-2">
          <button
            onClick={() => setActiveTab('pessoais')}
            className={`px-4 py-2.5 rounded-lg text-xs font-black tracking-wider uppercase transition-all duration-300 flex items-center gap-2 cursor-pointer ${
              activeTab === 'pessoais'
